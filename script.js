...
// PISCAR THEME
const words = document.querySelectorAll('.word-theme');
let isLocked = false;
let lockedScrollPos = 0;
const theme = document.querySelector(".theme-container");

// o bounding client rect é a partir do ecrã então é precisar somar o scroll
const themeOffset = theme.getBoundingClientRect().top + window.scrollY;
...
window.addEventListener("scroll", () => {
  ...
  // Logic triggers based on the section's position
  if (progress > (themeOffset + 900)) {
    if (words[0].innerHTML !== "resist") {
       lockAndTransform("resist", "original", themeOffset + 901);
    }
  } else if (progress > (themeOffset + 600)) {
    if (words[0].style.top !== "50%") {
       lockAndTransform(null, "50%", themeOffset + 601);
    }
  } else if (progress > (themeOffset + 300)) {
    if (words[0].innerHTML !== "reform") {
       lockAndTransform("reform", "original", themeOffset + 301);
    }
  } else if (progress > (themeOffset + 10)) {
    if (words[0].style.top !== "50%") {
       lockAndTransform(null, "50%", themeOffset + 11);
    }
  } else {
    // Reset when above the section
    updateWords("theme", "original"); 
  }
});

function lockAndTransform(text, topValue, scrollPoint) {
  isLocked = true;
  lockedScrollPos = scrollPoint; // Now snaps to the correct section position
